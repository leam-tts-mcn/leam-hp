#!/usr/bin/env python3
# Netlifyビルド時に実行：index.html からルート別HTML（title/description/OGP/canonical焼き込み）と
# sitemap.xml / robots.txt を生成する。SNSクローラー（JS非実行）にもページ別メタを返すため。
# メタ文言は index.html 内の JS `META` と同期させること。失敗時は非0終了でデプロイを止める。
import re
import sys
import datetime
